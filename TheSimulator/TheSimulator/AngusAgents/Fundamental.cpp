    ...
    if (msg->type == "EVENT_SIMULATION_START") {
        ...
        if (µ > uniform_dist(simulation()->randomGenerator())) {
            if (price_deviation > 0) {
                // Buy
                auto pptr = std::make_shared<PlaceOrderMarketPayload>(OrderDirection::Buy, DEFAULT_ORDER_VOLUME);
                simulation()->dispatchMessage(currentTimestamp, 1, name(), exchange_1, "PLACE_ORDER_LIMIT", pptr);
            } else {
                // Sell
                auto pptr = std::make_shared<PlaceOrderMarketPayload>(OrderDirection::Sell, DEFAULT_ORDER_VOLUME);
                simulation()->dispatchMessage(currentTimestamp, 1, name(), exchange_1, "PLACE_ORDER_LIMIT", pptr);
            }
